...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 0; i < n; i++) {
            int X1 = ID(xs, nx, X0[i]);
            int X2 = ID(xs, nx, X1[i]);
            int Y1 = ID(ys, ny, Y0[i]);
            int Y2 = ID(ys, ny, Y1[i]);
            int Z1 = ID(zs, nz, Z0[i]);
            int Z2 = ID(zs, nz, Z1[i]);
            for (int X = X1; X < X2; X++)
                for (int Y = Y1; Y < Y2; Y++)
                    for (int Z = Z1; Z < Z2; Z++) color[X][Y][Z] = 1;
        }
